...
@section('content')
{!! Breadcrumbs::render('location.page', $country, $location) !!}
<h3>Pages de la destination : {{$location->name}} 		{!! Formatter::button(route('front.location.id', [$location->id]), 'info', 'entypo-eye', 'Voir')!!}</h3>

		{!! Formatter::addButton(route('location.page.add', [$country->code, $location->id]))!!}
		...
		<table class="table table-bordered table-striped datatable" id="table">
			...
			<tbody>
			@foreach($location->pages as $page)
			    <tr>
					...
					<td>
						...
						{!! Formatter::button(route('location.page.visibility', [$country->code, $location->id, $page->id]), 'warning', 'fa-eye'.($page->visibility == 1 ? '-slash' : ''), str_replace('ée', 'er', $page->opposite()))!!}

						{!! Formatter::editButton(route('location.page.edit', [$country->code, $location->id, $page->id]))!!}
						...
						{!! Formatter::delete(route('location.page.delete', [$country->code, $location->id, $page->id]), $page->id, "Supprimer une page", "Voulez-vous vraiment supprimer la page " .$page->name ." ?" ) !!}
