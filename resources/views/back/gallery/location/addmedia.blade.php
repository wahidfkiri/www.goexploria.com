...
@section('content')
{!! Breadcrumbs::render('location.gallery.addmedia', $gallery->id) !!}
<h4>Ajout de médias à une galerie ({{ $gallery->name }})</h4>
<div class="row">
	<div class="col-md-12">
		...
		<div class="panel-body">
            <div class="col-md-4">
      <div class="details">
        <label title="max_file_uploads">Nb. de fichier maximum</label>: <?php echo ini_get('max_file_uploads'); ?>
        <br>
      ...
      {{ Form::open(array('route' => array('location.gallery.addmedia.post'), 'method' => 'POST', 'files'=>true, 'id' => 'uploadForm', 'autocomplete' => 'off', 'class' => 'form-horizontal form-groups-bordered')) }}
        ...
        <div class="col-md-4">
            ...
            {{ Form::open(array('route' => array('location.gallery.addvideo.post'), 'method' => 'POST', 'id' => 'uploadForm_video', 'autocomplete' => 'off', 'class' => 'form-horizontal form-groups-bordered')) }}
      ...
      <br><br>
      {!! link_to_route('location.gallery.edit', 'Retour à la galerie', [$gallery->id], ['class' => 'btn btn-default btn-sm', 'title' => 'Retour']) !!}          
      &nbsp;&nbsp; 
      {!! link_to_route('location.gallery.search', 'Liste des galeries', [], ['class' => 'btn btn-default btn-sm', 'title' => 'Retour']) !!}
      <br><br>
      
      {{ Form::open(array('route' => array('location.gallery.editmedia.post'), 'method' => 'POST', 'id' => 'editForm', 'class' => 'form-horizontal form-groups-bordered')) }}
          ...
          @foreach( $medias as $key => $media )
              <div class="container" ng-controller="GalleryBackCtrl">
                  ...
                  <div class="content">
                      ...
                      {!! Formatter::delete(route('location.gallery.deletemedia', [$media->id]), $media->id, "Supprimer un media", "Voulez-vous vraiment supprimer ce média <strong>" .$media->name ."</strong> (ID: ".$media->id.") ?" ) !!}
                  </div>
              </div>
          @endforeach
      </div>      
