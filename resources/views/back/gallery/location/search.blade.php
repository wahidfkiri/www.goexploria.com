...
@section('title', 'Galeries de destinations')
@section('content')
{!! Breadcrumbs::render('location.gallery') !!}
<h3>Galeries de destinations</h3>

		{!! Formatter::addButton( route('location.gallery.add', []) ) !!}
    ...
    <div class="panel panel-primary" data-collapsed="0">
			...
			<div class="panel-body">
				<table class="table">
					...
					<tr>
						<td>{{ Form::label('lname', 'Destination') }}</td>
						<td>{{ Form::text('lname', isset( $request->lid ) ? App\Models\Location::find($request->lid)->name : null, ["data-column"=>"1", 'id' => 'col1_filter', 'class' => 'form-control column_filter', 'placeholder' => 'Entreprise']) }}</td>
					</tr>
					<tr>
			...
			@foreach($galleries as $gallery)
			    <tr>
			      <td class="search-name cpointer">{{$gallery->gname}}</td>
  					<td>@foreach($gallery->locations->unique() as $location)<div class="item search-lname cpointer" title="{{$location->name}}">{{$location->name}}</div> @endforeach</td>
  					...
  					<td>
  						...
  						{!! Formatter::editButton(route('location.gallery.edit', [$gallery->id] )) !!}
  						
              {!! Formatter::button(route('location.gallery.addmedia', [$gallery->id] ), 'primary', 'entypo-picture', 'Médias') !!}
              
  						{!! Formatter::deleteButton($gallery->id)!!}
  						{!! Formatter::delete(route('location.gallery.delete', [$gallery->id]), $gallery->id, "Supprimer une galerie", "Voulez-vous vraiment supprimer la galerie <strong>" .$gallery->name ."</strong> incluant tous les médias associés, dans toutes les destinations liées ?" ) !!}
