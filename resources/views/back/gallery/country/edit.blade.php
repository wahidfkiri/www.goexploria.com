...
@section('content')
{!! Breadcrumbs::render('country.gallery.edit', $gallery) !!}
		...
		<div class="row">
			<div class="col-md-12">
				...
				<div class="panel-body">
				  ...
				  <div class="details">
				    <label>Pays liées à cette galerie</label>
				    <br>
				    @if( count_of($countries) == 0 )
				      Aucune
				    @endif
				    
				    @foreach( $countries as $country )
				      {{ $country->name }} ({{ $langList[$country->pivot->language_id] }})<br>
				    @endforeach
				  </div>

					{{ Form::open(array('route' => array('country.gallery.edit.post', $gallery->id), 'method' => 'post', 'id' => 'editForm', 'class' => 'form-horizontal form-groups-bordered')) }}
          ...
          <div style="position: relative;">
            ...
            <select id="sel_locations" multiple="true" class="form-control" name="locations[]" style="overflow-y:auto; max-width: 400px; position: absolute; top:56px; left:410px; border: 0;" size="10">
              @foreach($countries->unique() as $key => $country)
          ...
          --}}
					...
					&nbsp;&nbsp; <a href="{{ route('country.gallery.search') }}">Annuler</a>
